...
def identifier_par_score(
    ...
    config: dict,
    seuil: int = 4,
    retour_score: Literal[False] = False,
...
def identifier_par_score(
    ...
    for cle, data in config.items():
        ...
        for mot, valeur_mot in mots_cle.items():
            ...
            if (
                len(mot_normalise) < 3
            ):  # éviter que les mots trop court génère des faux positifs (ex: UBS dans sUBScription)
                nombre_occurrences = len(
                    re.findall(rf"(?<!\w){re.escape(mot_normalise)}(?!\w)", texte)
                )
            else:
                nombre_occurrences = len(
                    re.findall(rf"\b{re.escape(mot_normalise)}\b", texte)
                )
    ...
    )  # transforme le dictionnaire en liste trié selon le nombre d'occurrence.
    ...
    resultat = (
        gagnant if meilleur_score >= seuil and ecart >= seuil_confiance else "inconnu"
    )
